...
class GptImage2Error extends Error {
  constructor(message, exitCode, details = {}) {
    super(message);
    this.name = "GptImage2Error";
...
async function generateImage(options) {
  const args = {
    ...
    timeoutSec: (options && options.timeoutSec) || 300,
    force: Boolean(options && options.force),
  };

  if (!args.prompt) {
    throw new GptImage2Error("Missing prompt", 2);
  }
  if (!args.out) {
    throw new GptImage2Error("Missing out", 2);
  }
  if (!Number.isFinite(args.timeoutSec) || args.timeoutSec <= 0) {
    throw new GptImage2Error("timeoutSec must be a positive number", 2);
  }
  ...
  const refs = args.refs.map(resolveInputPath);
  for (const ref of refs) {
    if (!fs.existsSync(ref) || !fs.statSync(ref).isFile()) {
      throw new GptImage2Error(`Reference image not found: ${ref}`, 4, { ref });
    }
  }

  const outPath = validateOutputPath(args.out);
  if (!args.force && fs.existsSync(outPath)) {
    throw new GptImage2Error(`output file already exists: ${outPath}`, 2, {
  ...
  if (result.code !== 0) {
    throw new GptImage2Error(`codex exec failed (exit=${result.code}).`, 5, {
      result,
    });
  }
  ...
  if (ownSessions.length === 0) {
    throw new GptImage2Error(
      `No new matching session rollout file under ${sessionsRoot}`,
      6,
      { result, sessionsRoot },
    );
  }
  ...
  if (!image) {
    throw new GptImage2Error(
      "Image payload not found in any new session file",
      7,
      { result },
    );
  }
  ...
  if (!args.force && fs.existsSync(finalOutPath)) {
    throw new GptImage2Error(`output file already exists: ${finalOutPath}`, 2, {
      existingPath: finalOutPath,
    });
  }
